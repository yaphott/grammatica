...
char* grammatica_or_render(GrammaticaContextHandle_t ctx, const Or* or_expr, bool full, bool wrap) {
    ...
    if (!result) {
        if (rendered_quantifier) free(rendered_quantifier);
        return NULL;
    }
    size_t pos = 0;
    bool found = false;
...
static Grammar* or_simplify_subexprs(GrammaticaContextHandle_t ctx, Grammar** original_subexprs, size_t original_num, Quantifier quantifier) {
    ...
    if (n == 1) {
        ...
        if (quantifier.lower == 0 && quantifier.upper == 1) {
            if (subexprs[0]->type == GRAMMAR_TYPE_AND || subexprs[0]->type == GRAMMAR_TYPE_OR) {
                ...
                if ((sub_quant.lower == 0 && sub_quant.upper == 1) ||
                    (sub_quant.lower == 1 && sub_quant.upper == 1)) {
                    ...
                    if (sub_subexprs) {
                        ...
                        for (size_t i = 0; i < sub_num; i++) {
                            if (sub_subexprs[i]) grammatica_grammar_destroy(ctx, sub_subexprs[i]);
                        }
                        free(sub_subexprs);
                        return result;
                    }
        ...
        for (size_t i = 0; i < n; i++) {
            if (subexprs[i]) grammatica_grammar_destroy(ctx, subexprs[i]);
        }
        free(subexprs);
        return result;
    }
