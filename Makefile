...
format :
	...
	${PYTHON} -m isort --sp pyproject.toml -l ${EXTENDED_LINE_LENGTH} tests docs scripts
	${PYTHON} -m black --config pyproject.toml src
	${PYTHON} -m black --config pyproject.toml -l ${EXTENDED_LINE_LENGTH} tests docs scripts
	@echo 'Done'
